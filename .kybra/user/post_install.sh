...
exec 3>&1; output=$(KYBRA_VERSION=0.5.2 $global_kybra_cargo_bin run --manifest-path=.kybra/user/kybra_post_install/Cargo.toml user canisters/user.did 2>&1 1>&3 3>&-); exit_code=$?; exec 3>&-; if [ $exit_code -ne 0 ]; then echo "$output"; exit $exit_code; fi
